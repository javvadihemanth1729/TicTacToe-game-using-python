...
class TicTacToe:
    def __init__(self):
        ...
        for i in range(3):
            row=[]
            for j in range(3):
                buttons=tk.Button(self.window,text="",width=20,height=10,command=lambda i=i,j=j : self.make_move(i,j) )
    ...
    def make_move(self,row,col):
        if self.board[row][col]=="":
            ...
            if self.check_winner(self.current_player):
                mb.showinfo("Game Over!!" ,"The winner is "+str(self.current_player))
                self.window.quit()
            elif self.is_draw():
                mb.showinfo("Game Over!!","Its a Draw match")
                self.window.quit()
    ...
    def check_winner(self,player):
        ...
        if player==self.board[0][0]==self.board[1][1]==self.board[2][2]:
                return True
        if player==self.board[0][2]==self.board[1][i]==self.board[2][0]:
                return True
        return False
    def is_draw(self):
